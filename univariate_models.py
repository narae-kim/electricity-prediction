...
for house in ['3422', '3668', '3737', '3851']:
    ...
    train_df = df_train.iloc[:, 0:1]
    validate_df = df_validate.iloc[:, 0:1]
    test_df = df_test.iloc[:, 0:1]

    # training / validation
    training_set_scaled = sc.fit_transform(np.array(train_df))
    X_train, y_train = cu.split_univariate_X_y(training_set_scaled, LAGS)
    X_train_ann = np.reshape(X_train, (-1, X_train.shape[1]))
    X_train_deep = np.reshape(X_train, (X_train.shape[0], X_train.shape[1], n_features))
    X_train_cnnlstm = X_train_deep.reshape((X_train_deep.shape[0], n_seq, n_steps, n_features))

    validation_set_scaled = sc.transform(np.array(validate_df))
    X_validate, y_validate = cu.split_univariate_X_y(validation_set_scaled, LAGS)
    X_validate_ann = np.reshape(X_validate, (-1, X_validate.shape[1]))
    X_validate_deep = np.reshape(X_validate, (X_validate.shape[0], X_validate.shape[1], n_features))
    ...
    # test
    test_set_scaled = sc.transform(np.array(test_df))
    X_test, y_test = cu.split_univariate_X_y(test_set_scaled, LAGS)
    X_test_svm = X_test
    X_test_ann = np.reshape(X_test, (-1, X_test.shape[1]))
    X_test_deep = np.reshape(X_test, (X_test.shape[0], X_test.shape[1], n_features))
    ...
    predicted_y_ann = sc.inverse_transform(predicted_y_ann)
    ...
    CNN_model.add(Conv1D(filters=64, kernel_size=2, activation='relu', input_shape=(X_train_deep.shape[1], 1)))
    ...
    predicted_y_cnn = sc.inverse_transform(predicted_y_cnn)
    ...
    LSTM_model.add(LSTM(units=50, return_sequences=True, input_shape=(X_train_deep.shape[1], 1)))
    ...
    predicted_y_lstm = sc.inverse_transform(predicted_y_lstm)
    ...
    predicted_y_cnnlstm = sc.inverse_transform(predicted_y_cnnlstm)
    ...
    predicted_y_cnnlstm.index = test_df.index

    # plot
    fig = plt.figure(figsize=[8, 3])
    ax = fig.add_subplot(111)
    plt.plot(test_df.index, test_df, color='k', linewidth=1.5)
    plt.plot(predicted_y_svm.index, predicted_y_svm, color='g', linewidth=1)
    plt.ylabel('Electricity Usage (kWh)', fontsize=11)
    plt.xlim([test_df.index[LAGS], test_df.index[-1]])
    plt.legend([f'Actual - house {num}', 'SVM'], loc='best', fontsize=10)
    ax.xaxis.set_major_formatter(dates.DateFormatter('%b %d'))
    plt.savefig(f'plots/{PLOT_DIR}/univariate_timeseries_svm_{house}.png')
    ...
    ax.xaxis.set_major_formatter(dates.DateFormatter('%b %d'))
    plt.savefig(f'plots/{PLOT_DIR}/univariate_timeseries_ann_{house}.png')
    ...
    ax.xaxis.set_major_formatter(dates.DateFormatter('%b %d'))
    plt.savefig(f'plots/{PLOT_DIR}/univariate_timeseries_cnn_{house}.png')
    ...
    ax.xaxis.set_major_formatter(dates.DateFormatter('%b %d'))
    plt.savefig(f'plots/{PLOT_DIR}/univariate_timeseries_lstm_{house}.png')
    ...
    ax.xaxis.set_major_formatter(dates.DateFormatter('%b %d'))
    plt.savefig(f'plots/{PLOT_DIR}/univariate_timeseries_cnnlstm_{house}.png')
    ...
    ax.xaxis.set_major_formatter(dates.DateFormatter('%b %d'))
    plt.savefig(f'plots/{PLOT_DIR}/univariate_timeseries_models_{house}.png')
    plt.close('all')
